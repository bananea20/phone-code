...
class TextRegexFilter : AbsTextFilter() {
    ...
    private val regexList: List<Pair<Regex, List<String>>> = listOf(
        ...
        Pair(Regex("^[A-Z]\\d{2,5}"), listOf("B123", "X30336")),
        Pair(
        ...
        Pair(
            Regex("^\\d{1,2}-{1,3}\\d{2,4}"),
            listOf("1-067")
        ),
        Pair(
            Regex("^\\d{1,2}[A-Z]\\d{2,4}"),
            listOf("2A39")
        ),
        Pair(Regex("^[A-Z]\\d{1,2}-{1,3}\\d{2,4}[A-Z]"), listOf("B3-78A")),
        Pair(Regex("^[A-Z]\\d{1,2}[A-Z](-{1,3}| {1,4})\\d{2,4}"), listOf("B2D-012", "B2G 015")),
    )
